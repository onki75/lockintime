...
| - | [ゲーミフィケーション](./p2/gamification.md) | P2 / P2+ / P3 |
...
- データエクスポート/インポート（JSON形式、トライアル終了時のデータ保護用）
...
### P2: 高度な機能（Pro機能）
体験の強化。
- 位置情報制限 / ストリーク強化（草ヒートマップ + 数値 + マイルストーン + 修復）
- レスキューパス（一時解除 + ストリークフリーズの2用途）
- ロックモード・チャレンジ・Nuclear Option（3段階保護）
- 利用統計・インサイト（週次サマリー・サイト別統計）

### P2+: ゲーミフィケーション拡充（Pro機能）
エンゲージメントの強化。
- デイリーチャレンジ（3段階: ブロンズ / シルバー / ゴールド）
- セレブレーション演出（ブロック成功カウンター、チェスト開封、紙吹雪）

### P3: クラウド同期 + マスコット
デバイス間同期 + 育成要素。
- Firebase（Firestore）でリアルタイム同期
- 同期対象: ブロックリスト・ストリーク・DailyStats・表示設定
- マスコット育成（成長・進化・感情デザイン）
- UIモード切替（マスコットモード / シンプルモード）
- レスキューパスの第3用途（マスコットの餌）

### Future: スコープ外
- マスコットカスタマイズ（色、アクセサリー等）
- SNS共有（マイルストーン達成時）
- 週次レポート
- 集中モード（ポモドーロタイマー）
...
// ストリーク記録（3状態: success=成功, bypass=一時解除/修復, failure=失敗）
type StreakStatus = 'success' | 'bypass' | 'failure'
...
1ルールに複数の制限を設定した場合、**全て並列でAND評価**する。
どれか1つでもブロック条件に該当すればブロック。

**評価方式:**
- 各制限を独立して評価
- `full_block` がある → 他の制限に関係なく常時ブロック
- `time_of_day(9-18)` + `daily_duration(30分)` → 9-18時はブロック、18時以降も累計30分でブロック（両方独立に効く）
- `delay` + 他の制限 → 他の制限でブロックされない場合のみdelayが動作

**例:**
- `full_block` + `delay` → `full_block` で常にブロック、`delay` は実質無効
- `time_of_day(9-18)` + `daily_count(3回)` → 9-18時はブロック、18時以降も3回超過でブロック
- `location(職場)` + `daily_duration(30分)` → 職場ではブロック、職場外でも30分超過でブロック
