...
export function ScreenTimeSection({
  ...
  const exceeded = todayMinutes >= goal
    ...
    <section className="rounded-2xl border border-gray-200 bg-white px-4 py-3 shadow-sm">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <p className="text-sm font-semibold text-gray-900">今日の残り</p>
          <div className="group relative">
            ...
            <div className="pointer-events-none absolute bottom-full left-1/2 z-10 mb-1.5 -translate-x-1/2 whitespace-nowrap rounded-lg bg-gray-900 px-2.5 py-1.5 text-[11px] text-white opacity-0 shadow-lg transition-opacity group-hover:opacity-100">
              1日の目標: {formatMinutes(goal)}まで
            </div>
          </div>
        </div>
        <span className="text-xs text-gray-400">{formatMinutes(todayMinutes)} / {formatMinutes(goal)}</span>
      </div>
      ...
      <div className="mt-2">
        {exceeded ? (
          ...
          <p className={`text-center text-sm font-bold ${remaining <= 30 ? 'text-amber-600' : 'text-blue-600'}`}>
            あと {formatMinutes(remaining)}
          </p>
        )}
      </div>
